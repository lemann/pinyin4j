...
public class PinyinHelperTest extends TestCase
{
    ...
    /**
     * test for pinyin to Chinese
     */
    public void testPinyinToChinese()
    {
        /*
        {
            char[] expectedChineseArray = new char[] { '羻', '唴', '熗', '蹌',
                    '蹌', '跄', '炝', '嗆', '熗', '呛' };
            char[] chineseArray = PinyinHelper.pinyinToChinese("qiang4");

            for (int i = 0; i < expectedChineseArray.length; i++)
            {
                assertEquals(expectedChineseArray[i], chineseArray[i]);
            }

            assertEquals(expectedChineseArray.length, chineseArray.length);
        }
 **/       
        {
            char[] expectedChineseArray = new char[] {'的', '地', '得', '底'};
            char[] chineseArray = PinyinHelper.pinyinToChinese("de5");
        ...
        {
            char[] chineseArray = PinyinHelper.pinyinToChinese("lian3");
            
            for (int i = 0; i < chineseArray.length; i++)
            {
                System.out.println(chineseArray[i]);
            }
        }
    }
}
